...
# the named volume and hands ownership back.
seed_opencode() {
  need_cmd curl
  need_cmd tar
  ...
  docker compose run -T --rm --no-deps --user root \
    ...
    --entrypoint sh eru -c \
    'mkdir -p /opt/opencode/.opencode/bin && cp /tmp/opencode-bin /opt/opencode/.opencode/bin/opencode && chmod 755 /opt/opencode/.opencode/bin/opencode && chown -R 1000:1000 /opt/opencode && HOME=/tmp /opt/opencode/.opencode/bin/opencode --version'
  rm -rf "$tmp"
}
...
EOF
    prompt_required ERU_GITHUB_APP_ID "GitHub App ID (numeric)"
    prompt ERU_GITHUB_APP_INSTALLATION_ID "Installation ID (empty = auto-detect single install)"
  ...
  if [[ -n "$(load_env_value "$ROOT/.env" ERU_GITHUB_APP_ID)" ]]; then
