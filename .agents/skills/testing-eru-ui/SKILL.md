...
- Layout-chain gotcha: the React island mounts into a plain `<div id="ask-root">` inside `.ask-island`. When asserting "pinned footer / internal scroll" layouts, check the mount div participates in the CSS height/flex chain — an unstyled block mount div silently breaks `flex:1`/`min-height:0` children (e.g. `.ask-app`), so the composer floats under content or gets clipped by `overflow:hidden` instead of pinning. A quick `curl .../assets/eru.css | grep '#ask-root'` tells you if a rule exists; verify the desired flex rule (e.g. `display:flex; min-height:0`) is actually shipped rather than assumed.
