...
// Confidential client: the single-use short-lived state plus the exact registered
// redirect URI carry the CSRF/code-injection role, and the client secret
// authenticates the exchange. No scope is requested, but GitHub may return
// previously granted scopes for repeat authorizations — which is why the token
// is used once for identity and then discarded. Mirrors maomao's oauth.ts.
...
export async function fetchGithubUser(
  ...
  return {
    ...
    login: body.login,
    avatarUrl:
      typeof body.avatar_url === "string" && body.avatar_url.startsWith("https://") ? body.avatar_url : null,
  };
}
