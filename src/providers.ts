 ...
 * (default `~/.local/share/opencode/auth.json`) on startup; Eru's OpenCode
 * children inherit the process env, so writing the file the child resolves
 * needs no env plumbing and the key never transits `.env`.
 *
